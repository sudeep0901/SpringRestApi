...
import com.sudeep.rest.service.CustomerPageSortService;
...
public class CustomerController {
	...
	void customersPageable() {
		
		Pageable pageable = new PageRequest(0, 5);
		List<Customer> customers = customerService.paginateAllCustomers(pageable);
...
//		return null;
	...
	@RequestMapping("/serach/{search}")
	public List<Customer> getByMobile( @PathVariable(value="search") String search) {
	  ...
	  @RequestMapping(value = "/all", method = RequestMethod.GET)
	    ...
	    public ResponseEntity<?> createCustomer(@RequestBody Customer customer, UriComponentsBuilder ucBuilder) {
	        ...
	        if (!customerService.checkCustomerByMobile(customer.getMobile())) {
	        	...
	        	logger.error(Utillities.valueOf(customerService.checkCustomerByMobile(customer.getMobile())));
	        	
	            logger.error("Unable to create. A User with name {} already exist", customer.getMobile());
	            return new ResponseEntity(new CustomErrorType("Unable to create. A Customer with mobile" + 
	            customer.getMobile()+ " already exist."),HttpStatus.CONFLICT);
	        }
